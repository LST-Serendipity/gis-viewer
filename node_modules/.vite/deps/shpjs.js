...
function inverse3(p2) {
  ...
  if (!this.es) {
    ...
    if (Math.abs(phi) < HALF_PI) {
      ...
      lon = adjust_lon_default(this.long0 + d * (1 - ds / 6 * (1 + 2 * t + c2 - ds / 20 * (5 + 28 * t + 24 * ts + 8 * c2 * t + 6 * c2 - ds / 42 * (61 + 662 * t + 1320 * ts + 720 * ts * t)))) / cos_phi, this.over);
    } else {
...
function inverse17(p2) {
  var x = p2.x;
  var y = p2.y;
  p2.x = adjust_lon_default(this.long0 + (x - this.x0) / (this.a * this.rc), this.over);
...
function inverse26(p2) {
  ...
  if (Math.abs(con) <= EPSLN) {
    if (lat0 >= 0) {
      lon = adjust_lon_default(long0 + Math.atan2(p2.x, -p2.y), this.over);
    } else {
      lon = adjust_lon_default(long0 - Math.atan2(-p2.x, p2.y), this.over);
    }
    p2.x = lon;
  ...
  lon = adjust_lon_default(long0 + Math.atan2(p2.x * sinz, rh * this.cos_p14 * cosz - p2.y * this.sin_p14 * sinz), this.over);
  p2.x = lon;
  p2.y = lat;
  return p2;
}
...
function* p(r, t = l) {
  ...
  for (e < 0 && c(2), o = a(10), o - a(8) && c(3), e = f(16); o--; ) {
    ...
    e = m, s = i(30 + a(26) + a(28), b), yield { filename: g, comment: x, read: () => n - 8 ? n ? c(1) : s : t(s) }, e = U;
  }
}
